...
const UPDATE_STATS = (_, pool, liveStats, latency, mutation) => {
  ...
  if (_.heightData.last === undefined) {
    ...
    if (_.heightData.seen.length > 70) _.heightData.seen.splice(70, _.heightData.seen.length - 70);
  }
...
const Explorer = (
  ...
  <explorer>
    ...
    <mean-time>{(computedStats.totalTime/Math.min(70, stats.result.blocks.length)).toFixed(2)}</mean-time>
  </explorer>
);
